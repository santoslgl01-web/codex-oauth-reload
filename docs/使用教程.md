...
![05d15cd026ec21d0cffde04ad0d1a6c9](https://key.jcid.xyz/content-assets/library/2026/04/20260424-163252--05d15cd026ec21d0cffde04ad0d1a6c9--0a59ba73acff.png)
3. 将里面的内容全部清空，替换为下方脚本代码。
![2026 04 25 003728](https://key.jcid.xyz/content-assets/library/2026/04/20260424-163745--2026-04-25-003728--59bf89f34367.png)
...
![2026 04 25 003703](https://key.jcid.xyz/content-assets/library/2026/04/20260424-163745--2026-04-25-003703--21e892504b82.png)

### 第九部分：启用插件内的本地 Clash 桥接

Ultra1.5 起，插件侧边栏新增 `Clash 桥接` 开关。它不会替代 Clash Verge / Mihomo 本身的系统代理或 TUN，而是通过本机外部控制器自动切换节点。

#### 准备 Clash

1. 打开 Clash Verge / Mihomo 设置。
2. 开启 `外部控制器`，常见地址为 `127.0.0.1:62754`。
3. 如果配置了 `secret`，记下这个 Secret。
4. 开启 `系统代理` 或 `TUN`。
5. 确认 `chatgpt.com`、`auth.openai.com`、`accounts.openai.com`、`openai.com` 等规则会走代理节点。

#### 插件侧边栏设置

1. 打开插件侧边栏。
2. 找到 `Clash 桥接`，打开开关。
3. `控制器` 填：`http://127.0.0.1:62754`。
4. `Clash 分组` 填你当前用于选择节点的分组名，默认是 `NODE-SELECT`。
5. `Secret 可留空`：如果 Clash 没有设置 secret，就留空；如果有 secret，就填写对应值。

#### 自动切换逻辑

- 每完成一轮自动运行后，插件会请求本地 Clash API。
- 插件会尝试把 Clash 切到 `rule` 规则模式。
- 插件会读取配置的分组，并切换到下一个非香港、非内置、非流量套餐类节点。
- 最后一轮不会再切换，避免无意义操作。
