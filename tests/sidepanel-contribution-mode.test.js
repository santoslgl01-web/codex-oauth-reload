...
test('contribution mode manager enters mode, starts main auto flow, polls contribution status, and exits cleanly', async () => {
  ...
  const manager = api.createContributionModeManager({
    ...
    constants: {
      contributionPortalUrl: 'https://key.jcid.xyz',
      contributionUploadUrl: 'https://key.jcid.xyz/upload',
      pollIntervalMs: 2500,
    },
  });
  ...
  assert.deepStrictEqual(openedUrls, ['https://key.jcid.xyz']);
  ...
  assert.deepStrictEqual(openedUrls, ['https://key.jcid.xyz', 'https://key.jcid.xyz/upload']);
