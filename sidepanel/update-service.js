(() => {
  const GITHUB_OWNER = 'santoslgl01-web';
  const GITHUB_REPO = 'codex-oauth-reload';
  ...
  async function requestExtensionUpdate(options = {}) {
    if (!chrome?.runtime?.sendMessage) {
      throw new Error('浏览器扩展消息通道不可用');
    }

    const response = await chrome.runtime.sendMessage({
      type: 'REQUEST_EXTENSION_UPDATE',
      source: 'sidepanel',
      payload: {
        reason: options.reason || 'sidepanel_update_card',
        latestVersion: String(options.latestVersion || '').trim(),
      },
    });
