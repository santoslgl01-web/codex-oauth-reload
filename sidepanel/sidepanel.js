...
let runtimeUpdateRequestInFlight = false;
...
function getRepositoryHomeUrl() {
  ...
  return 'https://github.com/santoslgl01-web/codex-oauth-reload';
}
...
function getUpdateTargetUrl(snapshot = currentReleaseSnapshot) {
  return snapshot?.logUrl
    || snapshot?.releasesPageUrl
    || sidepanelUpdateService?.releasesPageUrl
    || getReleaseListUrl();
}

function setRuntimeUpdateButtonBusy(isBusy) {
  if (!btnOpenRelease) {
    return;
  }
  btnOpenRelease.disabled = Boolean(isBusy);
  btnOpenRelease.textContent = isBusy ? '检查中...' : '一键更新';
}

async function requestRuntimeExtensionUpdate(snapshot = currentReleaseSnapshot) {
  if (runtimeUpdateRequestInFlight) {
    return;
  }

  const targetUrl = getUpdateTargetUrl(snapshot);
  if (!sidepanelUpdateService?.requestExtensionUpdate) {
    showToast('一键更新能力不可用，已打开 GitHub Release。', 'warn', 3600);
    openExternalUrl(targetUrl);
    return;
  }

  runtimeUpdateRequestInFlight = true;
  setRuntimeUpdateButtonBusy(true);
  showToast('正在通过浏览器更新通道检查新版本...', 'info', 1800);

  try {
    const result = await sidepanelUpdateService.requestExtensionUpdate({
      latestVersion: snapshot?.latestVersion || '',
    });
    if (result?.status === 'update_available') {
      showToast(result.message || '检测到新版本，扩展将自动应用更新。', 'success', 4200);
      return;
    }

    const message = result?.message || '当前安装方式无法自动应用更新。';
    showToast(`${message} 已打开 GitHub Release。`, 'warn', 6200);
    openExternalUrl(targetUrl);
  } catch (error) {
    showToast(`一键更新失败：${error.message}。已打开 GitHub Release。`, 'warn', 6200);
    openExternalUrl(targetUrl);
  } finally {
    runtimeUpdateRequestInFlight = false;
    setRuntimeUpdateButtonBusy(false);
  }
}
  ...
  switch (snapshot?.status) {
    case 'update-available': {
      ...
      if (btnOpenRelease) {
        ...
        btnOpenRelease.disabled = false;
        btnOpenRelease.textContent = '一键更新';
        btnOpenRelease.title = '通过浏览器更新通道检查并自动应用新版；不支持时会打开 GitHub Release。';
        btnOpenRelease.onclick = () => requestRuntimeExtensionUpdate(snapshot);
      }
      break;
    }
...
async function initializeReleaseInfo() {
  const fallbackReleaseUrl = sidepanelUpdateService?.releasesPageUrl || 'https://github.com/santoslgl01-web/codex-oauth-reload/releases';
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'EXTENSION_UPDATE_AVAILABLE': {
      const version = String(message.payload?.version || '').trim();
      showToast(
        version
          ? `检测到扩展新版本 ${version}，点击“一键更新”可自动应用。`
          : '检测到扩展新版本，点击“一键更新”可自动应用。',
        'info',
        4200
      );
      break;
