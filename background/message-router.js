  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(message, sender) {
      switch (message.type) {
        ...
        case 'REQUEST_EXTENSION_UPDATE': {
          const state = await getState();
          if (isAutoRunLockedState(state)) {
            throw new Error('自动流程运行中，请先停止流程再更新扩展。');
          }
          if (typeof requestExtensionUpdate !== 'function') {
            throw new Error('一键更新能力尚未接入。');
          }
          return await requestExtensionUpdate({
