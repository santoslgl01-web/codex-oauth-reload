...
- `background/clash-bridge.js`：本地 Clash / Mihomo 桥接模块，封装外部控制器地址、代理分组、Secret、候选节点过滤与 `/configs`、`/proxies/{group}` 调用；自动运行每轮结束后可切换到下一个非香港、非内置节点。
- `background/contribution-oauth.js`：贡献模式的公开 OAuth 流程模块，负责调用 `key.jcid.xyz` 的公开贡献接口、保存贡献会话运行态、在主 10 步流程里为步骤 7 提供贡献登录地址、在步骤 9/10 衔接 callback 捕获与兼容提交 `/oauth/api/submit-callback`，并把真实服务端状态映射回 sidepanel 运行态。
...
- `background/runtime-update.js`：扩展运行时更新模块，封装 `chrome.runtime.requestUpdateCheck()` 与 `chrome.runtime.reload()`，为侧边栏“一键更新”提供 CRX / `update_url` 安装版的自动应用更新能力；如果当前是开发者模式加载目录，会返回手动更新提示。
...
- `docs/热更新与GitHub发布说明.md`：说明一键更新能力、Chrome 扩展更新限制、GitHub Release 需要上传的 CRX / `updates.xml` 以及更新清单生成方式。
- `docs/使用教程.md`：面向使用者的补充教程文档，当前说明扩展更新流程、Clash Verge 的“非港轮询”配置步骤，以及 Ultra1.5 的插件内 Clash 桥接开关。
...
- `scripts/generate-update-manifest.js`：GitHub 自托管更新清单生成脚本，根据扩展 ID、CRX 下载地址和版本号生成 `updates.xml`，用于配合 `manifest.json` 的 `update_url` 做浏览器扩展更新。
...
- `sidepanel/contribution-content-update-service.js`：侧边栏贡献内容更新服务，负责拉取 `https://key.jcid.xyz/api/content-summary`、缓存公开公告/教程摘要，并输出可用于提示展示的 `promptVersion` 与最新更新时间。
